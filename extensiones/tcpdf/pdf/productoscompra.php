...
$sql = "SELECT p.nombre, v.*, SUM(v.cantidad) AS venta FROM compra_producto v INNER JOIN product p ON v.id_producto=p.id AND fecha  LIKE '%$fechaFinal%' GROUP BY v.id_producto";
...
$sql = "SELECT p.nombre, v.*, SUM(v.cantidad) AS venta FROM compra_producto v INNER JOIN product p ON v.id_producto=p.id AND fecha BETWEEN '$fechaInicial' AND '$fechaFinalMasUno' GROUP BY v.id_producto";
} else {
$sql = "SELECT p.nombre, v.*, SUM(v.cantidad) AS venta FROM compra_producto v INNER JOIN product p ON v.id_producto=p.id AND fecha BETWEEN '$fechaInicial' AND '$fechaFinal'GROUP BY v.id_producto";
}
}
...
$fecha = $value['fecha'];
$venta = number_format($value['cantidad']);
